//+------------------------------------------------------------------+
//|                                   LarryWilliams_XAUUSD_Short.mq5 |
//|           XAUUSD M5 Short - Best Strategy (SL=$12 TP=$80 12h)    |
//|                                                                  |
//|   Scoring EXACTLY matches backtest Python scripts (01-09)        |
//|   Direction(40) + MACD(30) + FVG(18) + Bonus(16) = max 100      |
//|   Signal: Score<=5 + Price<EMA200 -> Short 0.01 lot             |
//+------------------------------------------------------------------+
#property copyright "Rakuten EA"
#property version   "2.00"
#property description "Larry Williams 趨勢強度評分做空策略 (XAUUSD M5)"
#property description "回測最佳: SL=$12 TP=$80 12h | PF=2.01 | 月均~$459"
...
input int    InpScoreThreshold= 5;       // 評分閾值 (<=此值做空)
input double InpMaxSpread     = 0.50;    // 最大點差 $ (0=關閉)
input ulong  InpMagicNumber   = 20250706;// Magic Number
...
int OnInit()
{
   ...
   Print("═══════════════════════════════════════════════════════");
   Print("  Larry Williams XAUUSD Short EA v2.0");
